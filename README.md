...
El objetivo de esta etapa es transicionar del procesamiento en tiempo real (Tarea 2) al **análisis offline (Batch)** de grandes volúmenes de datos.
...
1.  **Tokenización:** Separación de respuestas en palabras individuales.
2.  **Normalización:** Conversión a minúsculas y eliminación de signos de puntuación (Regex).
3.  **Filtrado (Stopwords):** Eliminación de palabras comunes sin valor semántico (ej: "the", "and", "is") mediante un *Join* con una lista de exclusión.
4.  **Agregación:** Conteo de frecuencia de palabras (WordCount) y ordenamiento descendente.
